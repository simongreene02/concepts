...
class PostOrderDFSTest {
  ...
  @BeforeEach
  void setUp() {
    depthFirstSearch = new PostOrderDFS<>();
  }
  ...
  void traverse_3nodes() {
    ...
    assertThat(depthFirstSearch.traverse(rootNode)).containsExactly(2, 3, 1).inOrder();
  }
  ...
  void traverse_7nodes() {
    ...
    assertThat(depthFirstSearch.traverse(rootNode)).containsExactly(4, 5, 2, 6, 7, 3, 1).inOrder();
  }
