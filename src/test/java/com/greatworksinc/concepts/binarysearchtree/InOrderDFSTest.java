...
class InOrderDFSTest {
  ...
  @BeforeEach
  void setUp() {
    depthFirstSearch = new InOrderDFS<>();
  }
  ...
  void traverse_3nodes() {
    ...
    assertThat(depthFirstSearch.traverse(rootNode)).containsExactly(2, 1, 3).inOrder();
  }
  ...
  void traverse_7nodes() {
    ...
    assertThat(depthFirstSearch.traverse(rootNode)).containsExactly(4, 2, 5, 1, 6, 3, 7).inOrder();
  }
