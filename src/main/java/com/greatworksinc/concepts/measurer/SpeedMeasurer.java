...
public class SpeedMeasurer {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      commandList[i * 4 + 1] =
          new InsertCommand(
              new int[] {firstIndex, middleIndex, lastIndex}[i], UUID.randomUUID().toString());
      commandList[i * 4 + 2] =
          new RetrieveCommand(new int[] {firstIndex, middleIndex, lastIndex}[i]);
      commandList[i * 4 + 3] =
          new SearchCommand(baseList.get(new int[] {firstIndex, middleIndex, lastIndex}[i]));
    }
    Duration[][] timeTable = measureOperations(baseList, commandList);
    System.out.println(
        String.format("%45s %10s %10s %10s", "command name", "arrayList", "linkedList", "delta"));
    for (int i = 0; i < timeTable.length; i++) {
      String formattedResult =
          String.format(
              "%45s %10d %10d %10d",
              commandList[i].getName(),
              timeTable[i][0].getNano(),
              timeTable[i][1].getNano(),
              timeTable[i][1].getNano() - timeTable[i][0].getNano());
      System.out.println(formattedResult);
    }
  }

  private static Duration[][] measureOperations(
      ImmutableList<String> baseList, Command[] commandList) {
